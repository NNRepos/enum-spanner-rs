...
//		println!("keep level: {} curr: {:?} next: {:?} keep {:?}",level, dag_bitmap.get_level(level-1), next_level, keep);
	...
	pub fn get_statistics(&self) -> (usize, usize, f64, usize, usize, f64) {
		let (num_matrices, num_used_matrices, matrix_avg_size, matrix_max_size) = self.get_matrix_stats();

		(num_matrices, num_used_matrices, matrix_avg_size, matrix_max_size, self.get_max_width(), self.get_avg_width())
	}

	fn get_matrix_stats(&self) -> (usize, usize, f64, usize) {
		let (count, used_count, total_size, max_size) = MatrixIterator::init(self).fold((0,0,0,0), |(count, used_count, total_size, max_size), x| {
			let size = x.get_width() * x.get_height();

			(count + 1, used_count + if x.get_usage_count()>0 {1} else {0}, total_size + size, std::cmp::max(max_size, size))
		} );

		(count, used_count, total_size as f64 / count as f64, max_size as usize)
	}
